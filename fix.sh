...
do
    ...
    then
        ...
        if [[ -f "$local_apps$launcher" ]]
        then
            if [[ "$current" != "steam" ]]
            then
                # Local launchers
                if [[ -f "$current" ]]
                then # Verifies if icon exists to copy
                    if grep -Gq "Icon=$current$" "$local_apps$launcher"
                    then
                        echo "L: Fixing $name..."
                        if [[ ! -d "$local_icon" ]]
                        then
                            su -c "mkdir '$local_icon' -p" "$user"
                        fi
                ...
                # Steam launchers
                if [[ -f "$steam_icon" ]]
                then # Verifies if steam icon exists to copy
                    if grep -Gq "Icon=$current$" "$local_apps$launcher"
                    then
                        echo "S: Fixing $name..."
                        if [[ ! -d "$local_icon" ]]
        ...
        # Global launchers
        if [[ $mode != "local" ]] && [[ -f "$global_apps$launcher" ]]
        then
            if [[ -f "$current" ]]
            then # Verifies if icon exists to copy
                if grep -Gq "Icon=$current$" "$global_apps$launcher"
                then
                    echo "G: Fixing $name..."
    ...
    then
        # Local revert
        if [[ -f "$local_apps$launcher" ]] && [[ -f "$current" ]]
        then
            if grep -Gq "Icon=$new_icon$" "$local_apps$launcher"
            then
        ...
        # Steam revert
        if [[ -f "$local_apps$launcher" ]] && [[ -f "$steam_icon" ]]
        then
            if grep -Gq "Icon=$new_icon$" "$local_apps$launcher"
            then
        ...
        # Global revert
        if [[ $mode != "l-revert" ]] && [[ -f "$global_apps$launcher" ]] && [[ -f "$current" ]]
        then
            if grep -Gq "Icon=$new_icon$" "$global_apps$launcher"
            then
